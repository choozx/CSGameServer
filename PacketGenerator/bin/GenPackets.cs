...
class S_Test2 : IPacket
{
	public int testInt;

	public ushort Protocol { get { return (ushort)PacketID.S_Test2; } }
	...
	public ArraySegment<byte> Write()
	{
		...
		count += sizeof(ushort);
		success &= BitConverter.TryWriteBytes(s.Slice(count, s.Length - count), (ushort)PacketID.S_Test2);
		count += sizeof(ushort);
		success &= BitConverter.TryWriteBytes(s.Slice(count, s.Length - count), this.testInt);
		count += sizeof(int);
		success &= BitConverter.TryWriteBytes(s, count);
